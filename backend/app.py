...
def signup():
    ...
    if user:
        ...
        connection.commit()
        db_disconnect(cursor,connection)
        result = {"user_id": user_id, "email": email, "role": role, "name": name, "family": family, "address": address}
        return result,200
...
def login():
    ...
    if user:
        ...
        if db_password == password:
            ...
            db_disconnect(cursor,connection)
            # Generate token
            token = tokenlib.make_token({'user_id': user_id}, secret=TOKEN_SECRET)
...
def list_users():
    ...
    result = []
    for user in users:
        result.append({"user_id": user[0], "email": user[1], "role": user[3], "name": user[4], "family": user[5], "address": user[6]})
    return result,200
...
def search_books():
    ...
    if len(args) > 0:
        cursor.execute("SELECT * FROM books WHERE " + " AND ".join("{} = %s".format(k) for k in args.keys()), list(args.values()))
...
def list_books():
    ...
    result = []
    for book in books:
        result.append({"book_id": book[0], "title": book[1], "author": book[2], "year": book[3], "genre": book[4]})
    return result,200
...
def add_book():
    ...
    if not ('title' in request_data and 'author' in request_data and 'year' in request_data and 'genre' in request_data):
    ...
    connection.commit()
    db_disconnect(cursor,connection)
    result = {"book_id": book_id, "title": title, "author": author, "year": year, "genre": genre}
    return result,200
...
def order_book(book_id):
    ...
    connection.commit()
    db_disconnect(cursor,connection)
    result = {"order_id": order_id, "book_id": book_id, "user_id": user_id, "order_date": now}
    return result,200
...
def list_all_orders():
    ...
    result = []
    for order in orders:
        result.append({"order_id": order[0], "book_id": order[1], "user_id": order[2], "order_date": order[3]})
    return result,200
...
def add_comment(book_id):
    ...
    cursor.execute("SELECT * FROM comments WHERE book_id = %s AND user_id = %s", (book_id,user_id))
...
def list_comments(book_id):
    ...
    for comment in comments:
        result.append({"book_id": comment[0], "user_id": comment[1], "comment": comment[2]})
    return result,200
...
def update_comment(book_id):
    ...
    cursor.execute("UPDATE comments SET comment = %s WHERE book_id = %s AND user_id = %s", (comment,book_id,user_id))
    connection.commit()
    db_disconnect(cursor,connection)
    return book_id,200
...
def admin_delete_comment():
    ...
    # Delete comment from database
    cursor.execute("DELETE FROM comments WHERE book_id = %s AND user_id = %s", (book_id,user_id))
    connection.commit()
    db_disconnect(cursor,connection)
    return book_id,200
